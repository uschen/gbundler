...
func TestBundlerCount1(t *testing.T) {
	...
	handler := &testHandler{}
	b := bundler.NewBundler(handler.handleImmediate)
...
func TestBundlerCountSlowHandler(t *testing.T) {
	handler := &testHandler{}
	b := bundler.NewBundler(handler.handleSlow)
	b.BundleCountThreshold = 3
...
func TestBundlerErrors(t *testing.T) {
	...
	// memory.
	b := bundler.NewBundler(func([]int) { select {} })
	b.BundleByteLimit = 3
	b.BufferedByteLimit = 10

	if got, want := b.Add(1, 4), bundler.ErrOversizedItem; got != want {
	...
	if got, want := b.Add(5, 1), bundler.ErrOverflow; got != want {
		t.Fatalf("got %v, want %v", got, want)
	}
}
...
func TestModeError(t *testing.T) {
	...
	if got, want := b.AddWait(context.Background(), 0, 2), bundler.ErrMixedMethods; got != want {
		t.Fatalf("got %v, want %v", got, want)
	}
	// Call AddWait then Add on new Bundler.
	b1 := bundler.NewBundler(func([]int) {})
	b1.BundleByteLimit = 4
	b1.BufferedByteLimit = 4
	if err := b1.AddWait(context.Background(), 0, 2); err != nil {
		t.Fatal(err)
	}
	if got, want := b1.Add(0, 2), bundler.ErrMixedMethods; got != want {
		t.Fatalf("got %v, want %v", got, want)
	}
}
...
func TestConcurrentHandlersMax(t *testing.T) {
	...
	b := bundler.NewBundler(func(s []int) {
		mu.Lock()
		active++
...
func (t *testHandler) handleImmediate(b []int) {
	t.mu.Lock()
	defer t.mu.Unlock()
	t.b = append(t.b, b)
...
func (t *testHandler) handleSlow(b []int) {
	t.mu.Lock()
	defer t.mu.Unlock()
	t.b = append(t.b, b)
	t.t = append(t.t, time.Now())
...
func (t *testHandler) handleQuick(b []int) {
	t.mu.Lock()
	defer t.mu.Unlock()
	t.b = append(t.b, b)
	t.t = append(t.t, time.Now())
...
func BenchmarkBundlerAddAndFlushSlow1(bench *testing.B) {
	// Unbundled case: one item per bundle.
	handler := &testHandler{}
	b := bundler.NewBundler(handler.handleQuick)
	b.BundleCountThreshold = 1
	b.DelayThreshold = time.Second
